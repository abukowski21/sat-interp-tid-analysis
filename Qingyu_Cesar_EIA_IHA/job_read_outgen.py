...
for year in tqdm(years_tot):
    ...
    print(f"tec_g contains {len(tec_g)} items.")
    # Check if any item is None
    if any(item is None for item in tec_g):
        print("Warning: Some files did not return a valid DataFrame.")
    print('1')
    print(tec_g)
    ...
    grnd_temp = pd.DataFrame({'DT': sat_date, 'GDLAT': sat_glat, 'GLON': sat_glon, 'TEC': sat_tec, 'MLAT': sat_mlat, 'MLON': sat_mlon, 'MLT': sat_mlt, 'kp':grnd_tec0.kp})
